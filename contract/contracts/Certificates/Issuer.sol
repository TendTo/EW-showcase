...
contract Issuer is Ownable {
    ...
    constructor(uint256 _certificateTopic, address _registry) Ownable() {
        require(_registry != address(0), "Cannot use address 0x0");

        certificateTopic = _certificateTopic;

        registry = CertificateRegistryExtended(_registry);
    }
	...
	*/
    ...
    function revokeRequest(uint256 _requestId) external {
        ...
        require(_msgSender() == request.owner || _msgSender() == Ownable.owner(), "Unauthorized revoke request");
    ...
    function approveCertificationRequest(
        ...
        require(_msgSender() == owner(), "caller not owner or issuer");
